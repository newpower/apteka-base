...
type
  TZakaz = class(TForm)
    ...
    procedure gridmenuSelectCell(Sender: TObject; ACol, ARow: Integer;
...
begin
    ...
    begin
      ...
      if flag_new then
        ...
        gridmenu.Cells[1,0]:=IntToStr(ClientData.OrdersCount+1);
         gridmenu.Cells[0,0]:='Новая заявка';
         gridmenu.Cells[2,0]:='0';
...
var Text:TextFile;
str,num_zakaz,date_str,id_tovar,str2:string;
num_str:Integer;
sum:real;
begin
       ...
       //ShowMessage('FILE '+ExeDir+'Data\Nakl\nakl_'+IntToStr(ClientData.user_id)+'_'+str_file+'.xml');
        ...
        begin
          ...
          begin
            ...
            //Читаем сообщения вложенные в накладные
          if Pos('<text',str)=1 Then
          begin
            Delete(str,1,Pos('text="',str)+5);
            str2:= copy(str,1,Pos('"',str)-1);

             Delete(str,1,Pos('date="',str)+5);
            str2:= copy(str,1,Pos('"',str)-1)+' '+str2;
...
var Text:TextFile;
str,num_zakaz,date_str,str2:string;
sum:real;
begin
...
begin
  ...
  // if ClientData.OrdersCount > 19 then
  // begin
  //     gridmenu.RowCount:=19;
  //     gridmenu.Cells[0,i]:='Еще 20 заявок';
  // end
  // else
      gridmenu.RowCount:=ClientData.OrdersCount;


      gridmenu.ColWidths[0]:=170;
      gridmenu.ColWidths[1]:=0;
      gridmenu.ColWidths[2]:=0;
      i:=0;
      if gridmenu.Cells[0,0] = 'Новая заявка' then
      i:=1;
      i2:=0;
      While (ClientData.OrdersCount-i2 > 0)  do
      begin
         gridmenu.Cells[1,i]:=IntToStr(ClientData.OrdersCount-i2);
         gridmenu.Cells[0,i]:='Заявка № '+IntToStr(ClientData.OrdersCount-i2);
         gridmenu.Cells[2,i]:='1';
         i:=i+1;
         i2:=i2+1;
      end;


    OrdersNode.Expand(true);
 //  Zakaz.MainTree.Items[1].Expand(true);
...
begin
  Zakaz.Caption:='Электронный заказ товаров. Прайс лист от '+ClientData.PriceDate+'. Ваш уникальный номер: '+inttostr(ClientData.user_id);
 Label3.Caption:=ClientData.PersonalManadger;
       flag_new:=True;
         BuildTree;
         StringGrid1.Cells[0,0]:='№';
    StringGrid1.Cells[1,0]:='KT';
    StringGrid1.Cells[2,0]:='Товар';
    StringGrid1.Cells[3,0]:='Серия';
    StringGrid1.Cells[4,0]:='Заказ      кол-во';
    StringGrid1.Cells[5,0]:='Заказ               Цена';
    StringGrid1.Cells[6,0]:='Заказ               Сумма';

    StringGrid1.Cells[7,0]:='Накладная            цена';
    StringGrid1.Cells[8,0]:='Накладная   кол-во';
    StringGrid1.Cells[9,0]:='Накладная              сумма';
...
//--
if ((ARow > 0) and (StringGrid1.Cells[9,ARow] <> '') and (StringGrid1.Cells[6,ARow] <> StringGrid1.Cells[9,ARow])) then
  begin
...
//FillRect(cr);
//TextOut(cr.left+((cr.Right-cr.Left) div 2)-(TextWidth(Cells [acol,arow]) div 2), cr.Top,Cells[acol,arow]);

    FillRect(Rect);
     SetBkMode(Handle, TRANSPARENT);
    Rect.Right:=Rect.Right -2;
    DrawText(Handle, pchar(Cells[ACol, ARow]), -1, Rect, DT_SINGLELINE OR DT_VCENTER OR DT_RIGHT);
     end;
 end;
end;
...
procedure TZakaz.gridmenuSelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
  var
    FileName:string;
begin
      If gridmenu.Cells[2,Arow]='0' Then
  begin
    ...
    Button4.Visible:=true;
      ...
      If gridmenu.Cells[2,Arow]='1' Then
      begin

        FileName:=gridmenu.Cells[1,Arow]+'.xml';
