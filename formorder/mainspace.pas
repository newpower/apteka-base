     ...
     //
      ...
      dt.AddFormField('get_price', '1');
       ...
       dt.AddFormField('submit-url', '/admin/saveconf_sc.as');
        ...
        IdHTTP1.Post('http://apteka-rostov.nawww.ru/obmen.php',dt,LocalFile);
      //    IdHTTP1.Get('http://agro2b.nawww.ru/index_info.php',LocalFile); //
          IdHTTP1.Disconnect
     end;
     except
     flag_connect:=False;
     ShowMessage('Проблема связи, при подключении к серверу'#13#10);
...
begin
  ...
  try
    ...
    assignFile(PFile,ExeDir+'Data/Inbox/price.xml');
    ...
    begin
      ...
      //Записываем дату прайса
      if Pos('<price ',str)=1 Then
         begin
                Delete(str,1,Pos('datetimedoc="',str)+12);
                datetimedoc:= copy(str,1,Pos('"',str)-1);

                IniFile:=TIniFile.Create(ExeIni);
                Inifile.WriteString('Client','PriceDate',datetimedoc);
                ClientData.PriceDate:=datetimedoc;
                IniFile.Free;


           //     Delete(str,1,Pos('captionfirm="',str)+12);
           //     captionfirm:=copy(str,1,Pos('"',str)-1)+' ';

           //     Delete(str,1,Pos('comment="',str)+8);
           //     comment:=date_str+copy(str,1,Pos('"',str)-1);
          end;

          if Pos('<line',str)=1 Then
          begin

              Delete(str,1,Pos('frm_codtovar="',str)+13);
              Rec.code:= copy(str,1,Pos('"',str)-1);

              Delete(str,1,Pos('frm_tovarname="',str)+14);
              Rec.name:= copy(str,1,Pos('"',str)-1);
              ...
              Rec.Goden:= copy(str,1,Pos('"',str)-1);

              Delete(str,1,Pos('frm_numser="',str)+11);
              Rec.Seria:= copy(str,1,Pos('"',str)-1);


              try
              Delete(str,1,Pos('frm_prise="',str)+10);
              Rec.Price:= strtof(copy(str,1,Pos('"',str)-1));

              // Rec.Price:=strtof(DeleteDelimeter(';',str));
              Delete(str,1,Pos('frm_number="',str)+11);
              ...
              Delete(str,1,Pos('frm_gvp="',str)+8);
              Rec.ZN:= copy(str,1,Pos('"',str)-1);

              // Rec.ZN:=DeleteDelimeter(';',str);
              Delete(str,1,Pos('frm_regk="',str)+9);
              Rec.RegKR:= copy(str,1,Pos('"',str)-1);

              // Rec.RegKR:=DeleteDelimeter(';',str);
              Delete(str,1,Pos('frm_prise_min_cost="',str)+19);
              Rec.cost_min:= strtof(copy(str,1,Pos('"',str)-1));
              //Rec.cost_min:=strtof(DeleteDelimeter(';',str));
              Delete(str,1,Pos('frm_prise_max_cost="',str)+19);
    ...
    closeFile(PFile);
  //  DeleteFile(ExeDir+'Data/Inbox/price.xml');
